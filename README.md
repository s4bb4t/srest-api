...
- **Путь**: `http://51.250.113.72:8082/api/v1/swagger/index.html#`
- **Описание**: Эндпоинт Swagger документации.
...
- **Метод**: PUT
- **Описание**: Обновляет права пользователя по указанному ID.
- **Параметры**:
  - **id** (путь): ID пользователя.
  - **UserData** (тело запроса, обязательный): Обновленные данные прав пользователя.
    ```json
    {
      "isAdmin": true,
      "isBlocked": false
    ...
    ```
- **Ответы**:
  - **200 OK**: Права пользователя успешно обновлены.
    ```json
    {
      "id": 1,
      "username": "string",
    ...
    ```
  - **400 Bad Request**: Ошибка десериализации JSON запроса.
  - **400 Bad Request**: Неверный или отсутствующий ID.
  - **404 Not Found**: Пользователь не найден.
  - **500 Internal Server Error**: Внутренняя ошибка сервера.
